...
void tryInitFFT(const int N) {
  ...
  //added by massimo
  arm_rfft_fast_instance_f32 fast1;
  //arm_rfft_instance_f32 foo4;
  status = arm_rfft_fast_init_f32(&fast1,  N);
 // status   arm_rfft_fast_f32 (arm_rfft_fast_instance_f32 *S, float32_t *p, float32_t *pOut, uint8_t ifftFlag)
  Serial.print(N);Serial.print(": arm_rfft_init_f32, FFT allocation : "); Serial.print(status); Serial.print(", where success is "); Serial.println(ARM_MATH_SUCCESS); 
...
void loop() {
  ...
  if (count++ < 1) {
    const int N_N = 8;
    int32_t all_N[] = {16, 32, 64, 128, 256, 512, 1024, 2048, 4096};
    for (int i=0; i < N_N; i++) {
      tryInitFFT(all_N[i]);
    }
    delay(2000);
